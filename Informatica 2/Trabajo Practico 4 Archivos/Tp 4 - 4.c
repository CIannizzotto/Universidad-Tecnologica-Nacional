...
void guardarStockEnArchivo(const char *nombreArchivo);
void cargarStockDesdeArchivo(const char *nombreArchivo);
...
int main() {
	...
	do {
		...
		case 6:
			...
			break;
		case 7:
			guardarStockEnArchivo("stock.dat");
...
void guardarStockEnArchivo(const char *nombreArchivo) {
						...
						if (archivo == NULL) {
							printf("No se pudo abrir el archivo para guardar el stock.\n");
							return;
						}
						
						// Escribir el arreglo de estructuras en el archivo
						fwrite(stock, sizeof(struct Imagen), numImagenes, archivo);
						
						fclose(archivo);
						printf("Stock guardado exitosamente en el archivo.\n");
					}
void cargarStockDesdeArchivo(const char *nombreArchivo) {
	FILE *archivo = fopen(nombreArchivo, "rb"); // Abre el archivo en modo binario
	
	if (archivo == NULL) {
	printf("No se pudo abrir el archivo para cargar el stock.\n");
	return;
	}
	// Leer el arreglo de estructuras desde el archivo
	numImagenes = fread(stock, sizeof(struct Imagen), MAX_IMAGES, archivo);

	fclose(archivo);
	printf("Stock cargado exitosamente desde el archivo.\n");
}
